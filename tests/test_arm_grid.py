...
def test_result_paths_are_self_describing_and_unique_per_arm():
    ...
    assert len(paths) == 8
    assert any(p.endswith('rf_t7_d10_M25_independent.csv') for p in paths)
    assert any(p.endswith('rf_t7_d10_M25_joint-d002.csv') for p in paths)
    assert any(p.endswith('rf_t7_d10_M25_joint-dinf.csv') for p in paths)
